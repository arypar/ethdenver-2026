...
function ChartTooltip({ active, payload, metric }: { active?: boolean; payload?: Array<{ value: number; payload: ChartDataPoint }>; metric: string }) {
  ...
  return (
    <div className="rounded-lg border border-white/[0.1] bg-black/80 px-3 py-2 backdrop-blur-xl shadow-xl">
      ...
      <p className="text-[10px] text-white/40 mb-0.5">{point.time}</p>
      <p className="text-[14px] font-bold text-white">{formatValue(payload[0].value, metric as ChartConfig['metric'])}</p>
    </div>
  );
}
    ...
    if (config.chartType === 'bar') {
      return (
        <BarChart data={data} margin={margin}>
          ...
          <XAxis {...xAxis} />
          <YAxis {...yAxis} />
          <Tooltip content={<ChartTooltip metric={config.metric} />} cursor={{ fill: 'rgba(255,255,255,0.02)' }} />
    ...
    if (config.chartType === 'line') {
      return (
        <LineChart data={data} margin={margin}>
          ...
          <XAxis {...xAxis} />
          <YAxis {...yAxis} />
          <Tooltip content={<ChartTooltip metric={config.metric} />} />
          <Line
    ...
    <div className={`group rounded-2xl border bg-white/[0.04] backdrop-blur-xl transition-all duration-300 hover:border-white/[0.12] hover:bg-white/[0.06] ${flash ? 'border-[#FF007A]/40 shadow-[0_0_20px_rgba(255,0,122,0.15)]' : 'border-white/[0.08]'}`}>
        ...
        <div className="min-w-0 flex-1">
          {editing ? (
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] text-white/30">{config.metric} · {config.pool} · {config.range}</span>
      ...
      {chart.data.length > 0 && (
        <div className="flex items-center gap-8 border-t border-white/[0.06] mx-5 py-3.5 text-[12px]">
          ...
          <Stat label={config.range} value={`${stats.change24h >= 0 ? '+' : ''}${stats.change24h.toFixed(2)}%`} positive={stats.change24h >= 0} showIcon />
